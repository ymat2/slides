...
2. リモートリポジトリの変更を手元に反映させる

3. あえて競合を起こしてそれを解決してみる

参考
: [すずえもんの前回の資料](https://drive.google.com/drive/u/0/folders/1lQ1zf6SsRHXyMxO_0xsnGyuNEcFVxhOu)
...
`git merge` は複数のブランチを一本にまとめるときに使う。

fast-forward(早送り) マージ
:   `main` ブランチに変更がない場合、マージするブランチに追いつくだけでいい。
:   ![](image/ff_merge.png){width="450"}
...
:   `main` ブランチにも変更がある場合、両者の変更を取り込んだコミットをつくる。
:   ![](image/non_ff_merge.png){width="450"}


## `merge` の役割は「ブランチの統合」

Pull時の `git merge` は、手元の `main` ブランチに、`git fetch` で変更を反映したした「リモート追跡ブランチ」をマージする。

![](image/fetch_and_merge.png){width="900"}
...
リモートとローカルの両方の変更を取り込んだ場合、"merge commit"が自動的につくられる。
...
`git pull` をした時も同様。「いい感じに`merge`する」という部分は便利な反面、作るつもりのない余計なコミットが勝手にできてしまうのは避けたい。

↓

そこで、「`git pull`のときはfast-forwardでのmergeのみを試みて、できなければfetchまでで止める。」という設定をする。
